...
export const  HandleCreateComment:RouteHandler = async(req,res,next,app)=>{
...
export const HandleGetComments:RouteHandler = async(req,res,next,app)=>{
    ...
    if(req.query.only_count){
      ...
      try{
         const comments = await app.commentManager.GetCommentsForPost(post_id,limit,offset);
...
export const HandleDeleteComment:RouteHandler=async (req,res,next,app)=>{
    const user_id: number = Number(req.context.user_id);
    const post_id:bigint =BigInt(req.body.post_id);
...
export const HandleCommentReply:RouteHandler=async(req,res,next,app)=>{
    ...
    if (comment_id == undefined || comment_id == null) {
        return next(new Herror("comment id missing", HerrorStatus.StatusBadRequest));
     }
     try{
...
export const HandleDeleteCommentReply:RouteHandler=async(req,res,next,app)=>{
    const user_id:number = Number(req.context.user_id);
    const comment_id:bigint = BigInt(req.body.comment_id);
    const parent_comment_id:bigint = BigInt(req.body.parent_comment_id);

    if (comment_id == undefined || comment_id == null|| parent_comment_id == undefined || parent_comment_id ==null) {
        return next(new Herror("comment_id | parent_comment_id missing", HerrorStatus.StatusBadRequest));
     }
     try{
       await app.commentManager.DeleteReply(parent_comment_id,user_id,comment_id);
       app.SendRes(res,{
        status:HerrorStatus.StatusOK,
