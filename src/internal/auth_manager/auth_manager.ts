...
export default class AuthManager {
  ...
  CreateUser(
    ...
    return this.store.users.create({
      data: { 
        mail_id, password, subject_id, identity_provider,
        Profile:{
           create:
           { username }
        } 
  ...
  UpdatePassword(mail_id: string, password: string): Promise<EAuth> {
  ...
  UserRegister(
    ...
    otp:string,
    password: string
    ):Promise<{responseStatus:IResponse; userData?: EAuth; accessToken?: string }>{
      ...
      try {
        ...
        if(!user){
          ...
          if(valid_otp === otp){
            ...
            const user: EAuth = await this.CreateUser(mail_id,username,enc_password);
            const accessToken = await this.CreateSession(64,user);
            resolve({
              responseStatus:{statusCode:HerrorStatus.StatusCreated,message:"successful_signup"},
              userData:user,
              accessToken
             });
           }
  ...
  GoogleLogin(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        resolve({
            responseStatus:{statusCode:HerrorStatus.StatusOK, message:"successful_login"},
            userData:user,
            accessToken
            });
      }
     ...
     return new Promise(async(resolve,reject)=>{
        try{
          ...
          if(!user){
            ...
            resolve({
              responseStatus:{statusCode:HerrorStatus.StatusOK,message:"otp_generated"},
              userData:otp
            });
          }
          else{
  ...
  UpdateUserPassword(mail_id:string,
    otp:string,
    password: string
    ):Promise<{responseStatus:IResponse,userData?: EAuth; accessToken?: string }>{
      return new Promise(async (resolve, reject) => {
        try {
          ...
          if(!user){
            ...
            if(valid_otp==otp){
              const enc_password:string = await this.HashPassword(password);
              const user: EAuth = await this.UpdatePassword(mail_id,enc_password);
              const accessToken = await this.CreateSession(64,user);
              resolve({
                responseStatus:{statusCode:HerrorStatus.StatusCreated,message:"successful_signup"},
                userData:user,
  ...
  CreateSession(n: number, user:EAuth):Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        this.cache.Set(token, userStringified, 600);
        resolve(token);
      } catch (err) {
        reject(err);
      }
