...
export default class ExploreManager {
  ...
  constructor(
    ...
    this.store = store;
    (this.imageResolver = imageResolver), (this.imageStorage = imageStorage);
  }
  ...
  async GetUserSuggestions(limit: number, offset: number, user_id: number) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const ru: UserRecommendations = (await this.GetSuggestedUsers(
          user_id
        ...
        const tu: TrendingUsers[] = (await this.GetTrendingUsers(
          limit,
          offset
        )) as TrendingUsers[];
        ...
        const following_: Networks[] = (await this.GetFollowing(
          user_id
        )) as Networks[];
